    	...
    	public function CheckOtp(Request $request)
{
    ...
    try {
        // 🔹 Get active OTP pack (locked)
        $otpPack = DB::table('otp_sms')
            ->where('status', 1)
            ->lockForUpdate()
            ->first();
        ...
        if (!$otpPack) {
            ...
            return response()->json([
                'status'  => false,
                'message' => 'No active OTP pack found'
            ], 400);
        }

        // ❌ No OTP remaining
        if ($otpPack->remaining_otp <= 0) {
            DB::rollBack();
            return response()->json([
                'status'  => false,
                'message' => 'Your OTP pack has been exhausted. Please recharge.'
            ], 400);
        }

        // 🔻 Decrease remaining OTP
        DB::table('otp_sms')
            ->where('id', $otpPack->id)
            ->decrement('remaining_otp', 1);

        // 📝 INSERT OTP PACK HISTORY (NEW WORK ✅)
        DB::table('otp_pack_history')->insert([
            'otp_pack_id' => $otpPack->id,
            'mobile'      => $mobile,
            'used_at'     => now()
        ]);
        ...
        return response()->json([
            ...
            'remaining_otp' => $otpPack->remaining_otp - 1
        ]);
    ...
    public function signup_contact_support()
    {
        $data = DB::table('admin_settings')
            ->where('id', 27)
            ->where('title', 'sign_up_contact_support')
            ->value('longtext');
  ...
  /////type= 2
    ...
    $bets = $betsQuery->selectRaw('
        COALESCE(SUM(trade_amount),0) as total_bet,
    ...
    $chicken = $chickenQuery->selectRaw('
        COALESCE(SUM(amount),0) as total_bet,
        COALESCE(SUM(win_amount),0) as total_win,
        MAX(created_at) as last_played_at
    ')->first();
    ...
    $gameHistorySummary = collect([]);

    // ✅ Wingo
    if ($bets->total_bet > 0 || $bets->total_win > 0) {
        $gameHistorySummary->push([
            'game_name' => 'Wingo',
            ...
            'last_played_at' => $bets->last_played_at,
        ]);
    }

    // ✅ Aviator
    if ($aviator->total_bet > 0 || $aviator->total_win > 0) {
        $gameHistorySummary->push([
            'game_name' => 'Aviator',
            ...
            'last_played_at' => $aviator->last_played_at,
        ]);
    }

    // ✅ Chicken Road
    if ($chicken->total_bet > 0 || $chicken->total_win > 0) {
        $gameHistorySummary->push([
            ...
            'last_played_at' => $chicken->last_played_at,
        ]);
    }

    // ✅ Other games
    foreach ($otherGames as $game) {
        if ($game->total_bet > 0 || $game->total_win > 0) {
            $gameHistorySummary->push([
    ...
    // ✅ Latest first
    $gameHistorySummary = $gameHistorySummary
...
if (empty($userIds)) {
    return response()->json([
        'status' => true,
        'message' => 'Campaign exists but no users assigned',
        'data' => [
            'campaign' => $campaign,
            'users' => [],
            'summary' => []
...
// $yourCommission = round($yourCommission, 2);
   ...
   /* =========================
   COMMISSION (SAME AS campaign_analytics)
========================= */
...
$commissionPercent = 30; // SAME AS ANALYTICS
$yourCommission = 0;

if ($netPL < 0) {
    $yourCommission = abs($netPL) * ($commissionPercent / 100);
} elseif ($netPL > 0) {
    $yourCommission = -($netPL * ($commissionPercent / 100));
}
...
$yourCommission = round($yourCommission, 2);
    ...
    /* =========================
       FIRST DEPOSIT MAP
    ========================== */
    $firstDepositMap = DB::table('payins')
        ->select('user_id', DB::raw('DATE(MIN(created_at)) as first_date'))
        ->whereIn('user_id', $userIds)
        ->where('cash', '>', 0)
        ->groupBy('user_id')
    ...
    if (!empty($userIds)) {
        ...
        if ($netPL < 0) {
            // USER LOSS → CAMPAIGN PROFIT
            $totalCommission = abs($netPL) * ($commissionRate / 100);
        } elseif ($netPL > 0) {
            // USER PROFIT → CAMPAIGN LOSS
            $totalCommission = -($netPL * ($commissionRate / 100));
        } else {
            $totalCommission = 0;
        }
    ...
    $allTimeFirstDeposits = $firstDepositMap->count();
    ...
    // ✅ ALL TIME COMMISSION (30% BET LOSS ONLY)
    $allTimeTotalCommission = round($allTimeTotalLoss * 0.30, 2);
    ...
    for ($i = 0; $i < 7; $i++) {
        ...
        $dailyFirstDeposits = 0;
        foreach ($firstDepositMap as $uid => $firstDate) {
            if ($firstDate === $date) {
                $dailyFirstDeposits++;
            }
        }
        ...
        $dailyData[] = [
            ...
            'first_deposits' => $dailyFirstDeposits,
            ...
            'total_commission' => round($dailyLoss * 0.30, 2),
            'total_loss' => (float) $dailyLoss,
        ];
    }
...
foreach ($aviator as $a) {

    // 🔴 Debit entry (Bet Placed)
    $summary->push([
        'date_time'   => $a->created_at,
        'credit'      => 0,
        'debit'       => (float) $a->amount,
        'balance'     => null, // yahin baad me wallet balance laga sakte ho
        'description' => 'Aviator Bet Placed',
        'round'       => $a->game_sr_num,
    ]);

    // 🟢 Credit entry (Bet Won)
    if ($a->win > 0) {
        $summary->push([
            'date_time'   => $a->updated_at,
            'credit'      => (float) $a->win,
            'debit'       => 0,
            'balance'     => null,
            'description' => 'Aviator Bet Won',
            'round'       => $a->game_sr_num,
        ]);
    }
}
...
foreach ($chicken as $c) {
    ...
    $summary->push([
        ...
        'balance'     => null,
        'description' => 'Chicken Bet Placed',
        'round'       => $c->period_no,
    ]);
    ...
    if ($c->win_amount > 0) {
        $summary->push([
            'date_time'   => $c->updated_at,
            'credit'      => (float) $c->win_amount,
            'debit'       => 0,
            'balance'     => null,
            'description' => 'Chicken Bet Won',
            'round'       => $c->period_no,
        ]);
    }
}
...
// $giftClaims = DB::table('gift_claim')
//     ->select('gift_code', 'amount', 'status', 'datetime')
//     ->where('userid', $userId)          // ✅ only this user
//     ->orderBy('datetime', 'asc')        // ✅ datewise
//     ->get();
...
$giftClaims = DB::table('gift_claim')
    ->select(
        ...
        'amount',
        'status',
        DB::raw("ADDTIME(datetime, '05:30:00') as datetime")
    )
    ->where('userid', $userId)
...
// if ($giftClaims->isNotEmpty()) {
//     foreach ($giftClaims as $g) {

//         // OPTIONAL: agar sirf success dikhana ho to
//         // if ($g->status != 1) continue;

//         if ($g->amount > 0) {
//             $summary->push([
//                 'date_time'   => $g->datetime,
//                 'credit'      => (float) $g->amount,
//                 'debit'       => 0,
//                 'balance'     => null,
//                 'description' => 'Gift Claimed | Code: ' . $g->gift_code,
//                 'round'       => null,
//             ]);
//         }
//     }
...
public function login(Request $request)
{
    ...
    // If identity is mobile number → country_code required
    $validator->sometimes('country_code', 'required|string', function ($input) {
        return ctype_digit((string) $input->identity);
    });
   ...
   STEP 2 : Password Check
    ...
    /* ===============================
       STEP 4 : Notification Insert
    ================================ */
    $now = now()->format('Y-m-d H:i:s');

    DB::table('notifications_login')->insert([
        'user_id'    => $user->id,
        'name'       => 'LOGIN JANKAARI',
        'disc'       => "Aapka khaata $now par login kiya gaya hai. 
Yadi yeh aap nahi hain, turant support se sampark karein.",
        'created_at' => now(),
        'updated_at' => now(),
    ]);

    /* ===============================
       STEP 5 : Store IP Address
    ================================ */
    ...
    public function login_bdg(Request $request) {
...
// ✅ sirf LOSS type (1) ke liye loss dikhana hai
if ($coupon && $coupon->type == 1) {
    $responseData['last_7_days_loss'] = (float) $totalLoss;
}
...
return response()->json([
    ...
    // ✅ SPECIAL CASE: ID = 0 → Only APK Link Return
    if ($uid == 0) {
    ...
    public function slider_image_view()
    {
        $slider = DB::select("
            ...
            WHERE status = 1
              AND (
                  active_from IS NULL 
                  OR active_from <= CONVERT_TZ(NOW(), '+00:00', '+05:30')
              )

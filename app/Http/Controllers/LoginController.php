...
public function auth_login(Request $request)
{
    ...
    // ❌ Agent blocked by admin
    if ($login->role_id == 4 && $login->status == 0) {
        session()->flash('msg', 'Blocked by admin');
        return redirect()->route('login');
    }

    // ✅ Single login session handling
    ...
    session()->put('permissions', json_decode($login->permissions, true));
    session()->put('password_changed_at', $login->password_changed_at);
    session()->put('last_activity', time());

    // ✅ Redirect based on role
    // return ($login->role_id == 4)
    //     ? redirect()->route('agent.panels')

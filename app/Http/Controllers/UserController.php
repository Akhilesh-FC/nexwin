...
class UserController extends Controller
{
        	...
        	public function otpPackHistory(Request $request)
{
    $perPage = $request->get('per_page', 10); // default 10

    $history = DB::table('otp_pack_history')
        ->orderBy('used_at', 'DESC')
        ->paginate($perPage)
        ->appends(request()->query()); // 🔥 pagination ke sath dropdown value retain

    return view('otp_pack.history', compact('history', 'perPage'));
}
    ...
    public function illegalUsers()
    {
        ...
        return view('illegalbet.illegal_bet', compact('illegalUsers'));
    }
    ...
    public function illegal_single_Users($userid)
    {
        ...
        return view('illegalbet.wingoSingleillegal', compact('data'));
    }
    ...
    $status = $request->status ?? 1;
       // dd($status, $request->all());
        if ($request->status == 1) {
            $data = DB::table('bets')->where('userid', $user_id)->orderBy('id', 'desc')->get();
    ...
    public function paymode_toggleStatus($id)
{
    $mode = DB::table('pay_modes')->where('id', $id)->first();

    if (!$mode) {
        return back()->with('error', 'Pay mode not found');
    }

    DB::table('pay_modes')->where('id', $id)->update([
        'status' => $mode->status == 1 ? 0 : 1
    ]);
